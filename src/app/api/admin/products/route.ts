...
export async function POST(req: Request) {
  ...
  if (!taxRateId) return json({ ok: false, error: "Selectează TVA (tax_rate_id)." }, 400);
    ...
    const rows = await sql`
      WITH new_product AS (
        INSERT INTO product (
          ...
          buy_price_net, margin_pct,
          is_active
        )
        VALUES (
          ${sku}, ${slug}, ${name},
          ${description},
          ${brandId}::uuid, ${taxRateId}::uuid,
          ${code}, ${codeNorm}, ${externalCode},
          ${uom}, ${weightKg}, ${lengthMm}, ${widthMm}, ${heightMm},
      ...
      link AS (
        ...
        SELECT (SELECT id FROM new_product), cats.category_id
        FROM cats
        ON CONFLICT DO NOTHING
        RETURNING 1
      )
      SELECT (SELECT id FROM new_product) AS id
    `;
