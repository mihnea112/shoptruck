...
export async function GET(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    // Lines (NEW schema) — also fetch primary image path + SKU for PDF
...
export async function DELETE(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    const del =
      await sql`DELETE FROM offer WHERE id = ${id}::uuid RETURNING id`;
...
export async function PUT(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    if (!accountId)
      return json(
        { ok: false, error: "Selectează clientul (accountId)." },
        400,
      );
    ...
    const normalizedItems = items.map((i: any) => {
      ...
      const unitNet = Number(
        i?.unit_net ?? i?.unitNet ?? i?.price ?? i?.unit_price_net ?? 0,
      );
      ...
      const productId = i?.productId
        ...
        : i?.product_id
          ? String(i.product_id).trim()
          : null;
      const name = String(i?.name ?? "").trim();

      if (!name) throw new Error("Lipsă nume produs pe o linie.");
      if (!Number.isFinite(qty) || qty <= 0)
        throw new Error("Cantitate invalidă pe o linie.");
      if (!Number.isFinite(unitNet) || unitNet < 0)
        throw new Error("Preț invalid pe o linie.");
      if (!Number.isFinite(taxFrac) || taxFrac < 0 || taxFrac > 1)
        throw new Error("TVA invalid pe o linie.");
    ...
    const safeValidUntil = validUntil
      ? new Date(validUntil).toISOString()
      : null;

    // Run everything that touches the DB in a safe transaction (postgres.js)
    const result = await sql.begin(async (tx) => {
      const q = tx as unknown as typeof sql;
      // Validate offer exists
      const exists =
        await q`SELECT id FROM offer WHERE id = ${id}::uuid LIMIT 1`;
      if (!Array.isArray(exists) || exists.length === 0) {
        return { ok: false as const, status: 404, error: "Oferta nu există" };
      }

      // Validate account exists
      const acc =
        await q`SELECT id FROM account WHERE id = ${accountId}::uuid LIMIT 1`;
      if (!Array.isArray(acc) || acc.length === 0) {
        return { ok: false as const, status: 404, error: "Client inexistent." };
      }
      ...
      const engineCode = vehicle?.engine_code
        ? String(vehicle.engine_code).trim()
        : null;
      const year =
      ...
      await q`DELETE FROM offer_item WHERE offer_id = ${id}::uuid`;
      ...
      for (const it of normalizedItems) {
        await q`
          ...
          VALUES (
            ${id}::uuid,
            ${it.productId ? it.productId : null},
            ${it.name},
            ${it.qty},
            ${it.unitNet},
            ${it.taxFrac},
